...
const Signup = ({setLoginOpen, setIsLogged, onClose}) => {
  ...
  const validateForm = () => {
    ...
    if (!nameRegex.test(name)) {
      setErrorMsg("Name must be at least 3 characters and contain only alphabets.");
      return false;
    }
    ...
    if (!passwordRegex.test(password)) {
      setErrorMsg("Password must be at least 6 characters and include both letters and numbers.");
      return false;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(error);
      setErrorMsg('Signup failed! ' + (error.response?.data?.message || 'Try again.'))
    }
  }
    ...
    <div className="p-4 sm:p-8">
      ...
      <div className="flex flex-col gap-y-2">
          ...
          <p className="text-xs text-center text-red-600" id="password-error">
            ...
            <button type="submit" className="w-full py-2.5 px-4 text-lg font-medium pri-btn focus:outline-hidden disabled:opacity-50 disabled:pointer-events-none">Sign up</button>
          </div>
        </form>
      </div>
