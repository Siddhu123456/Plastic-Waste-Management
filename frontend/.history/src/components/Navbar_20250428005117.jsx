      ...
      >
        {suggestions.length > 0 ? (
          suggestions.map((suggestion, index) => (
            <div key={index} className="px-4 py-2 hover:font-bold cursor-pointer">
              {suggestion}
            </div>
          ))
        ) : (
...
const UserSubMenu = memo(({onClose}) =>{
  return(
    <>
      ...
      <div className="p-4 px-6">
            <Link to="/">
              <button className="w-full pri-btn py-1.5 lg:px-12 font-medium">
                Logout
              </button>
            </Link>
      </div>
...
const MobileMenu = memo(({ isOpen, navItems, isLogged, onClose, theme, setTheme, setLoginOpen, setAuthOpen }) => {
...
const Header = () => {
  const c_id = (localStorage.getItem('user').type)
  console.log(c_id)
      ...
      <div className="hidden lg:block relative" ref={userMenuRef}>
        ...
        >
          ...
          className={`
            ...
            `}
          >
            <UserSubMenu onClose={() => setIsUserMenuOpen(!isUserMenuOpen)} />
          </div>
      </div>
    )
  })
