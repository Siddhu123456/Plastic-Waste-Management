...
const SearchBar = memo(({ searchQuery, handleSearch, clearSearch, suggestions, loading }) => (
      ...
      >
        {suggestions.length > 0 ? (
          suggestions.map((suggestion, index) => (
            <Link onClick={clearSearch} to={`/product/${suggestion.pId}`} className='hover:font-bold'>
...
const Header = () => {
  ...
  const [isLogged, setIsLogged] = useState(!!user && Object.keys(user).length > 0);
  ...
  const navItems = [
    ...
    const fetchCartCount = async () => {
      try {
        const response = await axios.delete(`http://localhost:8080/api/cart/count`, {
          params: {
            userId,
          },
          });
