...
export default function Pickups() {
  ...
  const handleRowClick = (order) => {
    ...
    if (!selectedPickups.includes(orderId)) {
      setSelectedPickups(prev => [...prev, orderId]);
    } else {
      setSelectedPickups(prev => prev.filter(id => id !== orderId));
    }
  };
  ...
  const handleUpdateStatus = async () => {
    ...
    for (const orderId of selectedPickups) {
      const order = pickups.find(o => o.id === orderId);
      if (order && order.status !== 'delivered') {
        const newStatus = getNextStatus(order.status);
  
        try {
          await axios.put(`http://localhost:8080/api/pickups/update/${orderId}/status`, {
            status: newStatus
          });
        ...
        <table className="w-full table-auto divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {filteredPickups.map((order) => (
              <tr 
                key={order.id} 
                className={`${selectedPickups.includes(order.id) ? 'bg-blue-50' : 'hover:bg-gray-50'} ${order.status === 'delivered' ? 'cursor-default' : 'cursor-pointer'}`}
            ...
            <div className="p-3 sm:p-6 overflow-y-auto max-h-80vh">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
                ...
                <div>
                  ...
                  <span className={`px-1.5 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full 
                    ${viewPickup.status === 'preparing' ? 'bg-yellow-100 text-yellow-800' : ''}
                    ${viewPickup.status === 'shipped' ? 'bg-blue-100 text-blue-800' : ''}
                    ${viewPickup.status === 'out for delivery' ? 'bg-purple-100 text-purple-800' : ''}
                    ${viewPickup.status === 'delivered' ? 'bg-green-100 text-green-800' : ''}
                  `}>
