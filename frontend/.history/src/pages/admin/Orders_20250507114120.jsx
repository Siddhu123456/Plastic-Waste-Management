...
export default function OrdersPage() {
    ...
    const fetchOrders = async () => {
      try {
        ...
        setOrders(sampleOrders.map(order => ({
          ...
          products: [
            { name: 'Sample Product 1', quantity: 2, price: order.subtotal / 2 },
            { name: 'Sample Product 2', quantity: 1, price: order.subtotal / 4 }
          ]
        })));
  ...
  const handleUpdateStatus = async () => {
    ...
    for (const orderId of selectedOrders) {
      ...
      if (order && order.status !== 'delivered') {
        ...
        try {
          // Commented out for demo, uncomment for real API integration
  ...
  const OrderCard = ({ order }) => {
    return (
      <div 
        className={`border rounded-lg p-3 mb-3 ${selectedOrders.includes(order.id) ? 'bg-blue-50 border-blue-200' : 'bg-white'}`}
      >
    ...
    <div className="bg-white p-2 sm:p-4 rounded-lg shadow-sm mb-6 w-full">
      ...
      {viewOrder && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-2">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-lg md:max-w-2xl mx-auto overflow-hidden max-h-screen">
            ...
            <div className="p-3 sm:p-4 overflow-y-auto" style={{ maxHeight: "calc(100vh - 150px)" }}>
              ...
              <div className="border rounded-lg overflow-x-auto">
                <table className="w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {viewOrder.products && viewOrder.products.map((product, index) => (
                      <tr key={index}>
                        <td className="px-2 py-2 whitespace-nowrap text-xs sm:text-sm text-gray-700">{product.name}</td>
                        <td className="px-2 py-2 whitespace-nowrap text-xs sm:text-sm text-gray-700">{product.quantity}</td>
                        <td className="px-2 py-2 whitespace-nowrap text-xs sm:text-sm text-gray-700">${product.price.toFixed(2)}</td>
                        <td className="px-2 py-2 whitespace-nowrap text-xs sm:text-sm text-gray-700">${(product.price * product.quantity).toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className="bg-gray-50 px-3 py-2 flex justify-end gap-2 rounded-b-lg sticky bottom-0">
              <button 
