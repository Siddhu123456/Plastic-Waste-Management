...
const ProductForm = () => {
  ...
  const uploadImages = async (filesToUpload) => {
    ...
    try {
      const uploadedImages = await Promise.all(
        filesToUpload.map(async (fileObj, index) => {
  ...
  const saveProduct = async () => {
    ...
    // Check if we already have a primary image or will have one after upload
    const willHavePrimaryImage = images.some(img => img.primary) || 
      (selectedFiles.length > 0 && images.length === 0);
    
    if (!willHavePrimaryImage) {
              ...
              <div key={idx} className="relative">
                ...
                <div className="absolute bottom-0 left-0 right-0 bg-black bg-opacity-50 p-1">
                  <button 
                    ...
                    className="text-xs text-white hover:text-red-300 w-full text-center"
                  >
                    Remove
                  </button>
                </div>
