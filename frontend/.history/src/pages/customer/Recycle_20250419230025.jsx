...
const Recycle = () => {
  ...
  return (
    ...
    <div className="bg-white dark:bg-dtc rounded-md col-span-12 lg:col-span-6 p-4 sm:p-6">
      ...
      <label htmlFor="notes" className="block mb-1 text-tc dark:text-white">Special Notes</label>
      <textarea
          name="notes"
          id="notes"
          placeholder="Additional Notes (optional)"
          className="py-2 min-h-20 px-4 block w-full bg-ow dark:bg-dsc text-tc dark:text-ow border-ow dark:border-dsc border rounded-lg focus:outline-none focus:border-lgg focus:ring-lgg disabled:opacity-50 disabled:pointer-events-none dark:placeholder-neutral-500 dark:focus:ring-neutral-600"
          value={formData.notes}
          onChange={handleChange}
        />
        {formData.weight < minWeight ? <p className="text-center text-red-500">Minimum Weight {minWeight} kg</p>:''}
        <button type="button" onClick={()=>console.log(formData)} disabled={formData.weight < minWeight} className="disabled:bg-neutral-500 disabled:cursor-not-allowed px-4 py-2.5 w-full font-semibold tracking-wide pri-btn">
          Submit Request
        </button>
      </div>
    </div>      
