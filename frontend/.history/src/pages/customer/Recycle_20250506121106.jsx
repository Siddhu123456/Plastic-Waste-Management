...
const Recycle = () => {
  ...
  const handleSubmit = async () => {
    console.log(userId, selectedAddress.id, weight)
    try {
      ...
      alert(`Pickup created with ID: ${response.data.id}`);
      setWeight('')
    } catch (error) {
      console.error(error);
  ...
  return (
    ...
    <div className="bg-white dark:bg-dtc rounded-md col-span-12 lg:col-span-6 p-4 sm:p-6">
      ...
      />
        ...
        <button type="button" onClick={handleSubmit} disabled={weight < minWeight} className="disabled:bg-neutral-500 disabled:cursor-not-allowed px-4 py-2.5 w-full font-semibold tracking-wide pri-btn">
          Submit Request
        </button>
      </div>
    </div>      
