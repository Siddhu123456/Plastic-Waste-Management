...
export default function ProfilePage() {
  ...
  const userData = JSON.parse(localStorage.getItem('user'));
      ...
      const [plasticContributed, setPlasticContributed] = useState(0);
  
      const fetchPickups = async () => {
        try {
          const response = await axios.get(`http://localhost:8080/api/pickups/user/${userId}`)
          const allPickups = response.data;

          const totalWeight = allPickups
            .filter(pickup => pickup.status === 'pending')
            .reduce((sum, pickup) => sum + pickup.weight, 0);
      
          setPlasticContributed(totalWeight);
        } catch (error) {
  ...
  if(plasticContributed >= 100) badge ='Green Champ'
  else if(plasticContributed >= 50) badge = 'Green Hero'
  ...
  return (
    ...
    <div className="bg-white dark:bg-dtc rounded-2xl shadow-lg overflow-hidden">
      ...
      <div className="flex justify-center -mt-8 mb-2">
        <div className="w-20 h-20 rounded-full bg-gradient-to-br from-lgg to-[#018a4c] flex items-center justify-center uppercase text-white text-3xl font-bold shadow-lg border-4 border-white dark:border-dtc">
      ...
      <div className="px-6 py-2 mb-6">
        <div className="p-4 flex justify-center">
          <div className="text-center">
            ...
            <span className="block text-2xl font-bold text-lgg mt-1">{plasticContributed} kg</span>
          </div>
        </div>
      </div>
